...
from Evolo.util.density_estimator import KNearestNeighborDensityEstimator
from Evolo.util.ranking import FastNonDominatedRanking, StrengthRanking
from Evolo.util.replacement import JoinPopulationRankingAndDensityEstimatorReplacement
...
class RankingAndDensityEstimatorReplacementTestCases(unittest.TestCase):
    def test_should_replacement_return_the_list_if_the_offspring_list_is_empty(self):
        ...
        replacement = JoinPopulationRankingAndDensityEstimatorReplacement(ranking, density_estimator)
    ...
    def test_should_replacement_return_the_right_value_case3(self):
        ...
        for i in range(len(points_offspring_population)):
            offsprings.append(Solution(2, 2))
            offsprings[i].objectives = points_offspring_population[i]
        replacement = JoinPopulationRankingAndDensityEstimatorReplacement(ranking, density_estimator)
        result_list = replacement.replace(population, offsprings)
